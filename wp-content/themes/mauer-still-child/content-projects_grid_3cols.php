					...
					<div class="image-imaginary-square-and-title-holder">
						<div class="image-imaginary-square-and-title portfolio-project-in-grid">
							<a class="entry-thumb-link" href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>">
								...
								<!-- 
								Tags displaying is conditional on being called from portfolio, not tag.
								Added the associated brand names with an artist using tags
								Elipses are conditional on there being more than three brands associated with artist. -->
								<?php $display = get_query_var('on_tag_page');
								if (!$display) { ?>
								<p class="brand-tags"><?php the_tags(''); 
								$posttags = get_the_tags();
								if($posttags AND sizeof($posttags) > 3) {
									echo '...';
								} ?></p>
								<?php } ?>
							</a>
